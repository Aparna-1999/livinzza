      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 w-full">
        <div className="grid gap-12 lg:grid-cols-[1.1fr_0.9fr] lg:items-center lg:gap-8 xl:gap-12 min-h-[600px] w-full max-w-full min-w-0">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="mt-8 flex flex-wrap items-center gap-4 w-full min-w-0">
                  <Button href={sliderData[activeSlide].href} variant="primary" className="inline-flex items-center gap-2 rounded-full px-6 sm:px-7 py-3 sm:py-3.5 text-xs sm:text-sm font-semibold shadow-lg shadow-primary/20 shrink-0">
          ...
          <div className="relative flex items-center justify-center gap-3 sm:gap-6 lg:gap-8 py-6 w-full max-w-full overflow-hidden sm:overflow-visible">
            ...
            <div className="relative h-[280px] w-[135px] sm:h-[460px] sm:w-[240px] shrink-0 overflow-hidden rounded-3xl sm:rounded-[2.5rem] shadow-xl border border-slate-200/60 dark:border-white/10 bg-slate-100 dark:bg-slate-800">
              <AnimatePresence mode="wait">
                <motion.div
                  key={activeSlide}
              ...
              <div className="absolute inset-x-0 bottom-0 h-24 sm:h-32 bg-gradient-to-t from-slate-950/80 via-slate-950/30 to-transparent z-10" />

              {/* Slider Control Pill at the bottom */}
              <div className="absolute bottom-3 sm:bottom-5 left-1/2 z-20 flex -translate-x-1/2 items-center gap-1.5 sm:gap-3 rounded-full bg-primary px-3 sm:px-5 py-1.5 sm:py-2.5 text-white shadow-lg backdrop-blur scale-90 sm:scale-100 whitespace-nowrap">
                <span className="text-xs sm:text-sm font-bold tracking-wider">0{activeSlide + 1}</span>
                <div className="flex items-center gap-1 sm:gap-1.5 px-0.5 sm:px-1">
                  {sliderData.map((_, idx) => (
                    <button
                      key={idx}
                      onClick={() => setActiveSlide(idx)}
                      className={`h-1.5 rounded-full transition-all ${activeSlide === idx ? "w-3 sm:w-4 bg-white" : "w-1.5 bg-white/40 hover:bg-white/60"}`}
                ...
                <button
                  onClick={nextSlide}
                  className="flex h-6 w-6 sm:h-7 sm:w-7 items-center justify-center rounded-full bg-white/20 text-white transition-all hover:bg-white/40"
