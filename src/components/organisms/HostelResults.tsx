...
const HostelResults = ({ citySlug, collegeQuery, hostelQuery }: HostelResultsProps) => {
  const [searchVal, setSearchVal] = useState(hostelQuery || collegeQuery || citySlug || "");
  ...
  // 1. Search Query filtering (locality, name, city, college)
  if (searchVal.trim() !== "") {
    ...
    // If searchVal is empty, but we have URL params, use them as defaults
    if (citySlug) {
      filteredResults = filteredResults.filter((hostel) =>
        slugify(hostel.city).includes(slugify(citySlug))
      );
    }
    if (collegeQuery) {
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-8">
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          ...
          <aside className="w-full lg:w-72 xl:w-80 shrink-0 sticky top-24 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-2xl p-6 shadow-sm">
            ...
            {/* 2. Search Locality Input */}
            <div className="mb-6">
              <label className="text-[10px] font-bold text-slate-450 dark:text-slate-400 uppercase tracking-wider block mb-2">Search Locality / Hostel</label>
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Search locality, college..."
